...
def bfs_step(adjList, queue, visited, traversal_order, canvas, delay, status_label, image_references, goalNode):    
    if queue:
        currentNode = queue.popleft()
        traversal_order.append(currentNode)
        update_status_label(status_label, len(traversal_order), traversal_order, visited)

        create_image_case(canvas, 720, currentNode, image_references)

        if currentNode == goalNode:
            return  # Stop if the goal node is reached
        ...
        canvas.after(delay, bfs_step, adjList, queue, visited, traversal_order, canvas, delay, status_label, image_references, goalNode)
...
def main():
    ...
    startNode = 0  # Start node
    goalNode = 5   # Goal node
    root = tk.Tk()
